...
const HOME_BANNER_HEIGHT: u16 = 20;

pub fn render(app: &App, frame: &mut Frame) {
    match app.screen {
        Screen::Home => render_home(app, frame),
        Screen::Match => render_match(frame),
    }
}
...
fn render_home(app: &App, frame: &mut Frame) {
    ...
    frame.render_widget(
        Paragraph::new("Start Match").style(home_option_style(app.home_selected == 0)),
        chunks[2],
    );
    frame.render_widget(
...
fn render_match(frame: &mut Frame) {
    ...
    frame.render_widget(Paragraph::new("Press 'b' to return"), chunks[1]);
}
